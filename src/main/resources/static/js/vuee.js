...
Vue.component('options',{
    props:['opts'],
    template:`<select v-bind:opts="val" v-on:input="$emit('input', $event.target.value)">
<slot></slot>
<option v-for="index in range(0,10)" :value="index"">{{index}}</option>
</select>
`,
    data:function(){
        return {
...
var vm = new Vue({
    ...
    methods: {
        ...
        sav: function () {
            ...
            setTimeout(()=>{this.cli(this.vi)},500);
            setTimeout(()=>{this.cli(this.dimas)},100);
